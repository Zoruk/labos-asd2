...
{
    // Cette methode doit retourner la distance maximum entre le sommet v et 
    // le sommet le plus éloigné dans sa composante connexe
    int lastVisited = 0;
    
    // Avec un parcour BFS le noeud le plus loins sera visite en dernier
    BFS<Graph> bfs(G);
    bfs.visit(v, [&lastVisited] (int v) {
        lastVisited = v;
    });
    ...
    // on fait une boucle pour recuperer le parent jusqu'a ce qu'on retrouve le
    // sommet initial
    while (lastVisited != v) {
        lastVisited = bfs.parentOf(lastVisited); 
        ++dist;
    }
    return dist;
}
...
{    
    ...
    cout << message << " a " << G.V() << " sommets, " << CountEdges(G) 
            << " aretes et " << cc.Count() << " composantes connexes \n";
    cout << "le sommet " << 0 << " fait partie de la CC " << cc.Id(0) 
            << ". La distance maximale a ce sommet dans cette CC est de " 
            << FindMaxDistanceFrom(G,0) << "\n";     
    cout << "le sommet " << G.V()-1 << " fait partie de la CC " 
            << cc.Id(G.V()-1) << ". La distance maximale a ce sommet dans cette CC est de " 
            << FindMaxDistanceFrom(G,G.V()-1) << "\n";
...
int main(int argc, char** argv) {
    ...
    string filename("mediumG.txt"); // ou "mediumG.txt"
    ifstream s(filename);
