...
// unordered_map est plus rapide mais impose que std::hash<TYPE> sois existant et
// que l'operateur == de type existe aussi
...
class PriorityQueue {
    private:
        ...
        std::unordered_map<TYPE, size_type> keys;
    ...
    public:
        PriorityQueue(size_type size = 0) {
            queue.reserve(size + 1);
            queue.resize(1);
        }
        
        void insert(TYPE v, KEY k) {
            insert({v, k});
        }
        ...
        void decreaseKey(TYPE t, KEY key) {
